document.addEventListener('DOMContentLoaded', function () {
    let computers = []; // Array para armazenar os computadores localmente
    ...
    // Função para buscar computadores do banco de dados
    function fetchComputers() {
        fetch('http://localhost/gerenciamento/api.php') // Endpoint para buscar os dados do banco
    ...
    computerForm.addEventListener('submit', function (e) {
        ...
        if (editingIndex !== null) {
            ...
            fetch(`http://localhost/gerenciamento/api.php${computers[editingIndex].id}`, {
                method: 'PUT',
            ...
            fetch('/api/comphttp://localhost/gerenciamento/api.phputers', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(computer)
            })
    ...
    window.botaodlt = function () {
        ...
        if (confirm('Tem certeza que deseja excluir este computador?')) {
            ...
            const computerId = computers[index].id;

            fetch(`http://localhost/gerenciamento/api.php${computerId}`, { method: 'DELETE' })
                .then(() => {
                    fetchComputers();
